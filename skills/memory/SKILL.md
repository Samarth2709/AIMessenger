...
5. Use `history_search` and `history_read` only to recall exact private chat excerpts. History is not evidence for a semantic-memory write and must never be automatically promoted into the vault.
