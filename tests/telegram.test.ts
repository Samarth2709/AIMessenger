...
describe("TelegramClient", () => {
  ...
  it("advertises the cost command to Telegram", async () => {
    const fetchImpl = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const url = String(input);
      if (url.endsWith("/deleteWebhook")) return response({ ok: true, result: true });
      if (url.endsWith("/getMe")) return response({ ok: true, result: { id: 1, username: "testbot" } });
      if (url.endsWith("/setMyCommands")) {
        ...
        expect(body.commands).toContainEqual({
          command: "model",
          description: "Choose a Codex or gateway model",
        });
